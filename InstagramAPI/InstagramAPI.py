# -*- coding: utf-8 -*-
"""
Created on Mon Oct  1 15:59:49 2018

@author: Esat Kalfaoglu
"""
...
class InstagramAPI:
    ...
    def uploadPhoto2(self, photo, userTagDictList, location, caption=None, upload_id=None, is_sidecar=None):
        success=False
        if upload_id is None:
            upload_id = str(int(time.time() * 1000))
        data = {'upload_id': upload_id,
        ...
        response = self.s.post(self.API_URL + "upload/photo/", data=m.to_string())
        if response.status_code == 200:
            success = self.configure2(upload_id, photo, userTagDictList, location, caption)
            if success:
                self.expose()
        return success
    ...
    def uploadVideo2(self, video, thumbnail, userTagDictList, location, caption=None, upload_id=None, is_sidecar=None):
        success=False
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 200:
                time.sleep(60)
                success = self.configureVideo2(upload_id, video, thumbnail, userTagDictList, location, caption)
                if success:
                    self.expose()
    ...
    def configureVideo2(self, upload_id, video, thumbnail, userTagDictList, location, caption=''):
        clip = VideoFileClip(video)
        self.uploadPhoto(photo=thumbnail, caption=caption, upload_id=upload_id)
        data = json.dumps({
            'upload_id': upload_id,
            ...
            'usertags':json.dumps({'in': userTagDictList}),
            'video_result': 'deprecated',
            'clips': {
                'length': clip.duration,
                'source_type': '3',
    ...
    def configure2(self, upload_id, photo, userTagDictList, location, caption=''):
        (w, h) = getImageSize(photo)
        data = json.dumps({'_csrftoken': self.token,
                           'media_folder': 'Instagram',
                           'source_type': 4,
    ...
    def getStoryFeed(self,usernameId):
        return self.SendRequest('feed/user/' + str(usernameId) + '/story/')
    
    def getSelfStoryFeed(self):
        return self.getUserStoryFeed(self.username_id)
    ...
    def mute(self, userId):
        ...
        return self.SendRequest('friendships/mute_posts_or_story_from_follow/', self.generateSignature(data))
